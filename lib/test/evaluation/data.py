...
class Sequence:
    ...
    # Sequence(sequence_name, frames, 'vot', ground_truth_rect)
    def __init__(self, name, frames, dataset, ground_truth_rect, ground_truth_seg=None, init_data=None,
                 object_class=None, target_visible=None, object_ids=None, multiobj_mode=True):
        self.name = name
        self.frames = frames
        ...
        if init_data is not None:
            ...
            if self.ground_truth_rect is not None:
                if self.multiobj_mode:
                    ...
                    init_data[0]['bbox'] = OrderedDict({obj_id: list(gt) for obj_id, gt in self.ground_truth_rect.items()})
                else:
                    assert self.object_ids is None or len(self.object_ids) == 1
                    if isinstance(self.ground_truth_rect, (dict, OrderedDict)):

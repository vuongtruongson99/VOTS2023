...
def run_dataset(dataset, debug=False, threads=0, num_gpus=8):
    ...
    print('Evaluating trackers on {:5d} sequences'.format(len(dataset)))
    multiprocessing.set_start_method('spawn', force=True)
    ...
    if mode == 'sequential':
        for seq in dataset:
            trackers = [Tracker('procontext', 'procontext', 'vot2023', run_id) for run_id in range(len(seq.ground_truth_rect))]
            print(trackers)
